// M08 Set follow-up and M09 Update follow-up, shared by the Server Actions and the
// offline sync (M19).
...
import type { WriteContext } from "@/lib/writes/context";
...
export async function recordFollowUpResultCore(
  input: RecordFollowUpResultInput,
  user: SessionUser,
  ctx: WriteContext,
): Promise<{ followUpId: string; nextFollowUpId: string | null }> {
  const now = ctx.now;
