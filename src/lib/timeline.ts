...
// Who a row is by when no one on the staff did it: the app itself, setting an occasion
// follow-up (M23).
...
export async function writeTimelineEvent(
  ...
  input: {
    customerId: string;
    staffId: string | null; // null: nobody on the staff (an occasion follow-up set by the app)
    kind: TimelineKind;
    detail?: string;
