...
export type CustomerProfile = {
  ...
  // M16.02: whether they agreed, when, and which staff member recorded it.
  consent: { given: boolean; at: Date | null; byName: string | null };
  openEnquiry: {
    title: string;
...
export async function customerProfile(
  ...
  const consentBy = row.consentById
    ? await db.user.findUnique({ where: { id: row.consentById }, select: { fullName: true } })
    : null;
